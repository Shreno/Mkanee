...
class SubAmenitiesController extends Controller
{
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'name.fa' => 'required|string|max:191|unique:sub_amenities,name->fa',
        ...
        if ($request->has('options')) {
            foreach ($request->options['ar'] as $index => $optionAr) {
                ...
                $optionfa = $request->options['fa'][$index] ?? null;

                SubAmenityOption::create([
                    'sub_amenity_id' => $subAmenity->id,
                    'name' => ['ar' => $optionAr, 'en' => $optionEn,'fa'=>$optionfa],
                ]);
            }
        }
    ...
    public function update(Request $request,  $subAmenity)
    {
        ...
        $data = $request->validate([
            ...
            'name.fa' => 'required|string|max:191|unique:sub_amenities,name->fa,' . $subAmenity->id,
