        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                $.each(xhr.responseJSON.errors, function(key, value) {
                    ...
                    var fa_item = key.includes('.fa') ? key.replace(".fa", "[fa]") : key;

                    if (ar_item !== en_item && ar_item !== fa_item) {
                        ...
                        $('.store input[name="' + fa_item + '"]').addClass('border-danger');
                        $('.store input[name="' + fa_item + '"]').after(`<span class="mt-5 text-danger">${value}</span>`);
                        ...
                        $('.store select[name="' + fa_item + '"]').after(`<span class="mt-5 text-danger">${value}</span>`);
                        ...
                        $('.store textarea[name="' + fa_item + '"]').addClass('border-danger');
                        $('.store textarea[name="' + fa_item + '"]').after(`<span class="mt-5 text-danger">${value}</span>`);
                    } else {
                        // Handle cases where the language items are the same
                           $('.store input[name="' + ar_item + '"]').addClass('border-danger')
                            $('.store input[name="' + ar_item + '"]').after(`<span class="mt-5 text-danger">${value}</span>`);
                            $('.store input[name="' + en_item + '"]').after(`<span class="mt-5 text-danger">${value}</span>`);
                            $('.store input[name="' + fa_item + '"]').after(`<span class="mt-5 text-danger">${value}</span>`);
